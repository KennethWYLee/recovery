...
目前原始碼基準為產品/API `2.2.0`、schema `0004`。API、資安負例、agent 設計黑箱、短時讀取負載、受控 Log、資料庫未就緒後恢復、fresh-D1 runtime bootstrap 及瀏覽器查核所綁定的本機 Worker SHA-256 為 `e725a8a8c1cb9b0b41a1b478e6ad0ca6b11c515d673e051ced27c6c92429cedd`。可執行 Gherkin、規則故障注入、單元測試、品質指標、D1 還原、clean-room 與 manifest 也有目前來源的本機結果；各結果只支持明列的 artifact、來源、Log、migration 或本機環境，不能改寫為 CI、staging、production、正式獨立 QA 或外部使用證據。
...
| CO-VRF-QUALITY-001 | `evidence/continuity-ops-quality-metrics.json` | `verified_local`；37/37 單元測試；line 91.72%、branch 84%、function 92.66%；5 個複雜度診斷項目；CRAP 未計算 | 指定六個單元測試套件及六個領域或 bootstrap source 的整體 coverage 門檻與複雜度診斷；三項 aggregate coverage 均達既定門檻。 | 斷言品質、個別檔案都達 aggregate 門檻、Worker／UI／migration coverage、功能正確性、資安、可用性或發布準備度。 |
...
| CO-VRF-RUNTIME-BOOTSTRAP-001 | `evidence/continuity-ops-local-runtime-bootstrap.json` | `verified_local_controlled`；`passed_with_documented_limits`；3 phases；每階段 query 數 39／39／33；ready 後 3/3 核心讀取通過；Worker SHA-256 `e725a8a8...29cedd` | 隔離合成 fresh D1 在未授權身分下維持 503 且沒有 schema mutation；設定的 bootstrap administrator 可完成三階段初始化。最終狀態為 schema `0004`、14 tables／20 indexes／46 triggers、fingerprint `41532388...35bfc9`、FK 違反 0，並建立 1 位啟用中管理員。 | Sites 或 production hosted D1 已成功初始化、正式 identity forwarding、防偽與完整存取政策；Cloudflare production query 計數／方案上限、遠端併行與部分故障、CI、rollback 或發布核准。 |
...
| 項目 | 可核對位置 | 狀態 | 尚缺的證據 |
|---|---|---|---|
| 角色與事件授權 | `lib/operations-domain.ts`、`lib/operations-auth.ts`、`tests/operations-authorization.test.ts`、`tests/operations-domain.test.ts` | `verified_local`：整體單元關卡 37/37；本列只以其中對應的授權與領域案例，以及 Gherkin 選定情境、18/18 故障注入和指定 Worker API 正反例支持角色主張 | 正式身分邊界、完整遠端角色矩陣及正式獨立測試。 |
...
| Fresh-D1 runtime bootstrap | `db/operations-bootstrap-core.ts`、`db/operations-bootstrap.ts`、`tests/operations-bootstrap.test.mjs`、`scripts/run-local-runtime-bootstrap.mjs`、CO-VRF-RUNTIME-BOOTSTRAP-001 | `verified_local_controlled`：三階段 39／39／33 queries；未授權身分不得變更 schema；最終 14／20／46 inventory、指定 fingerprint、FK 違反 0 及 ready 後 3/3 讀取通過 | Sites／production hosted D1、正式 identity edge、防偽、平台實際 query 計數與上限、遠端重試／併行／部分故障及發布後 smoke。 |
