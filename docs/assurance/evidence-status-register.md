...
| D1 schema 與 migration | `drizzle/0001_continuity_ops_v2.sql` 至 `drizzle/0004_service_lifecycle_accountability.sql`、`tests/operations-migration.test.mjs` | `verified_local`：最終 15/15 migration 案例；另有 4/4 migration 的隔離還原 JSON | 遠端 D1、真實資料量、完整原始測試輸出檔、停機與 rollback。 |
