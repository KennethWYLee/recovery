...
目前原始碼基準為產品/API `2.2.0`、schema `0004`。既有 API、資安負例、agent 設計黑箱、短時讀取負載、受控 Log、資料庫未就緒後恢復及瀏覽器查核綁定 Worker SHA-256 `e725a8a8c1cb9b0b41a1b478e6ad0ca6b11c515d673e051ced27c6c92429cedd`；新的 fresh-D1 runtime bootstrap 與校內唯讀存取查核綁定 `930199c06dc8297377b5ab937cd5ad4105ff6d17ff6a6c94cd118a874199ac32`。兩組 artifact 證據不得混用。各結果只支持明列的 artifact、來源、Log、migration 或本機環境，不能改寫為 CI、staging、production、正式獨立 QA 或外部使用證據。
...
| CO-VRF-QUALITY-001 | `evidence/continuity-ops-quality-metrics.json` | `verified_local`；39/39 單元測試；line 92.05%、branch 85.43%、function 93.91%；5 個複雜度診斷項目；CRAP 未計算 | 指定六個單元測試套件及六個領域或 bootstrap source 的整體 coverage 門檻與複雜度診斷；三項 aggregate coverage 均達既定門檻。 | 斷言品質、個別檔案都達 aggregate 門檻、Worker／UI／migration coverage、功能正確性、資安、可用性或發布準備度。 |
...
| CO-VRF-RUNTIME-BOOTSTRAP-001 | `evidence/continuity-ops-local-runtime-bootstrap.json` | `verified_local_controlled`；`passed_with_documented_limits`；3 phases、39／39／33 queries、ready 後 3/3 核心讀取；校內唯讀 5/5 讀取與 4/4 method 拒絕；Worker SHA-256 `930199c0...99ac32` | 隔離合成 fresh D1 的三階段初始化、最終 14／20／46 inventory、指定 fingerprint 與 FK 0；兩個同時的校內首次請求只建立 1 user／1 membership／1 auto-provision audit；當次隨機角色為 `auditor`，5 個模組可讀，4 個狀態變更 method 均為 403／`READ_ONLY_ACCESS`，成員目錄為 403、稽核 actor email 省略；既有 admin 角色保留，其他網域未受邀者與 1 筆 suspended 會員為 403 | Sites／production hosted D1 或 identity forwarding、防偽、所有身分／角色組合、admin actor email 顯示、完整存取政策；Cloudflare production query 計數／方案上限、遠端併行與部分故障、CI、rollback 或發布核准。 |
...
| 項目 | 可核對位置 | 狀態 | 尚缺的證據 |
|---|---|---|---|
| 角色與事件授權 | `lib/operations-domain.ts`、`lib/operations-auth.ts`、`tests/operations-authorization.test.ts`、`tests/operations-domain.test.ts`、CO-VRF-RUNTIME-BOOTSTRAP-001 | `verified_local`／`verified_local_controlled`：39/39 單元中的授權案例涵蓋精確網域、兩個唯讀角色與 method 政策；10 個 Gherkin 情境含校內寫入拒絕及 observer 無指派讀取；隔離 runtime 另核對一個隨機角色的 5 個讀取模組、4 個 method 拒絕、成員目錄拒絕及稽核 Email 省略 | 正式身分邊界的實際第二個 NTUB 帳號、完整遠端角色矩陣、admin actor email 顯示及正式獨立測試。 |
...
| Fresh-D1 runtime bootstrap 與校內唯讀建立 | `db/operations-bootstrap-core.ts`、`db/operations-bootstrap.ts`、`db/operations.ts`、`scripts/run-local-runtime-bootstrap.mjs`、CO-VRF-RUNTIME-BOOTSTRAP-001 | `verified_local_controlled`：三階段 39／39／33 queries；最終 14／20／46 inventory、指定 fingerprint、FK 0、ready 後 3/3；另核對並行校內首次登入的單一 user／membership／audit、5/5 讀取、4/4 method 拒絕、資料最小化、既有 admin、其他網域與 suspended 會員 | Sites／production hosted D1、正式 identity edge、防偽、平台實際 query 計數與上限、所有身分／角色組合、遠端重試／併行／部分故障及發布後 smoke。 |
...
- 校內唯讀政策在 hosted edge 的精確網域、既有使用者／會員停用、兩個隨機角色、admin actor email 與完整回應欄位矩陣。
