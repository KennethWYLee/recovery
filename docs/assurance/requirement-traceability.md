...
|---|---|---|---|---|---|
| AC-001 | `/role-selection`、`/api/v1/session/role`、`/api/v1/access` 與所有受保護 API 的 actor 建立 | `authenticatedContext`、`isNtubEmail`、`isSchoolSelectableOrganizationRole`、`loadOrProvisionOperationsActor`、`schoolSessionRole`；管理員排除、會員版本與事件責任相容性查核 | `ops_users`、`ops_memberships`、`ops_incident_assignments`、`ops_audit_events`、`ops_idempotency_receipts`；唯一性、版本及角色相容 triggers | 授權／領域／建置測試；CO-VRF-RUNTIME-BOOTSTRAP-001 | `verified_local_controlled`：兩個同時首次請求只建立 1 user／1 membership／1 audit；回傳四種非管理員選項，拒絕 `admin`，完成 `commander → observer` 切換；既有 admin 保留，其他網域未受邀者與 1 筆 suspended 會員為 403。production 身分邊界未驗證。 |
| AC-002 | 所有受保護 API、存取政策、稽核與事件畫面 | `organizationRoleCanUseRequestMethod`、`actorPermissions`、`organizationRoleCanHoldIncidentRole`、`requirePermission`、角色選擇、欄位投影、事件角色及通訊檢查 | 會員、事件指派、相容性 triggers、最後管理員／指揮官 triggers、版本 write guard | 授權／領域測試；CO-VRF-RUNTIME-BOOTSTRAP-001 | `verified_local_controlled`：伺服器權限隨 `commander → observer` 切換；唯讀角色完成 5/5 讀取、4/4 method 拒絕、成員目錄 403，稽核未含 actor email。完整責任矩陣、正式身分邊界與 admin actor email 待查核。 |
