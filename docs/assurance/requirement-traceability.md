...
|---|---|---|---|---|---|
| AC-001 | `/api/v1/access` 與所有受保護 API 的 actor 建立 | `authenticatedContext`、`resolveExternalOperationsIdentity`、`isNtubEmail`、`provisioningRoleForIdentity`、`randomReadOnlyOrganizationRole`、`loadOrProvisionOperationsActor`；先查既有會員並在建立後重讀 | `ops_users`、`ops_memberships`、`ops_organizations`、`ops_audit_events`；Email、組織會員與自動建立 audit ID 唯一性 | `operations-authorization.test.ts`；CO-VRF-RUNTIME-BOOTSTRAP-001 | `verified_local_controlled`：隔離本機 Worker／D1 的兩個同時首次請求只建立 1 user／1 membership／1 audit；既有 admin 保留，其他網域未受邀者與 1 筆 suspended 會員為 403。當次角色為 `auditor`；使用者 suspended、全部相似網域與 production 身分邊界未由該 runtime 證據驗證。 |
| AC-002 | 所有受保護 API、存取政策、稽核與事件畫面 | `organizationRoleCanUseRequestMethod`、`actorPermissions`、`organizationRoleCanReadAllIncidents`、`requirePermission`、本人存取政策、actor email 投影、事件角色及通訊檢查 | 會員、事件指派、`ops_assignment_role_compatibility_insert_guard`、`ops_membership_assignment_compatibility_update_guard` 及最後管理員／指揮官 triggers | 授權／領域測試；CO-VRF-RUNTIME-BOOTSTRAP-001 | `verified_local_controlled`：當次隨機校內唯讀角色完成 5/5 讀取，4/4 狀態變更 method 為 403／`READ_ONLY_ACCESS`，成員目錄為 403，稽核未含 actor email。正式身分邊界仍須以實際第二個 NTUB 帳號驗證；admin actor email 亦待查核。 |
...
| AC-014 | `/api/v1/audit`、所有 API 回應 | `auditInsert`、`rejectedMutationAudit`、`access:manage` 控制的稽核回應投影、`emitOperationsRequestTelemetry`、RFC 7807 problem response | `ops_audit_events` append-only；時間軸 append-only；actor email 仍在使用者資料，不複製到 telemetry | bounded-route 單元案例；CO-VRF-API-001／SEC-001／TELEMETRY-001／CO-VRF-RUNTIME-BOOTSTRAP-001 | 既有受控 Log 為 815/815 有效；新的隔離本機 runtime 證據核對校內唯讀稽核回應省略 actor email。Admin actor email 顯示與正式管線未驗證。 |
...
| AC-017 | `/operations?view=&incident=&tab=` | `OperationsApp.tsx` 的網址狀態、輪詢、dialog focus；`globals.css` 響應式規則 | 不適用；資料授權仍由 API 執行 | `rendered-html.test.mjs`；CO-VRF-BROWSER-001 | 1265×513 與 360×844 應用程式視窗已核對深層連結、End／Home／方向鍵分頁切換、瀏覽器前進後退，以及 dialog／手機 drawer 焦點；Axe 選定規則為 0 violation、0 incomplete。本輪未重跑兩分頁輪詢；完整 WCAG、真實螢幕閱讀器、跨瀏覽器、真實裝置、正式獨立 QA 與遠端證據不足。 |
| AC-018 | 建置及交付流程 | `package.json` scripts、CI workflow、vinext／Worker 設定、manifest、telemetry analyzer 與 restore drill | migration 集合 | CO-VRF-CLEAN-ROOM-001／MANIFEST-001／D1-RESTORE-001 | 最終 Worker SHA-256、本機建置、4/4 整合及 0001→0004 還原已驗證；clean-room 對 Git 判定無變更且不含 ignored input 的 97 個來源檔執行 18 個命令，均 exit 0，API 71/71；manifest 盤點 15/15。這不是獨立 remote clone、CI、bit-for-bit 可重現、artifact rollback 或正式部署證據。 |
...
`CO-VRF-GHERKIN-FAULT-001` 以 6/6 個手工設計故障核對 AC-002、AC-004、AC-006、AC-008 與 AC-009 的選定情境判別力，0 survived。它只涵蓋目前 10 個情境中的六項風險，不是完整 mutation campaign、所有需求的證明或獨立人類 QA。
