...
| AC-002 | 所有受保護 API、存取管理與事件指派畫面 | `requirePermission`、`organizationRoleCanHoldIncidentRole`、`canTransitionIncident`、`assertResponder`、`assertCommander`、通訊編輯／核准檢查 | 會員、事件指派、`ops_assignment_role_compatibility_insert_guard`、`ops_membership_assignment_compatibility_update_guard` 及最後管理員／指揮官 triggers | 授權／領域／migration；CO-VRF-GHERKIN-001／FAULT-001／API-001／SEC-001 | 本機 37/37 單元、migration、選定 Gherkin、18/18 故障注入與指定 Worker API 負例已驗證；正式身分邊界未驗證。 |
...
| AC-014 | `/api/v1/audit`、所有 API 回應 | `auditInsert`、`rejectedMutationAudit`、`emitOperationsRequestTelemetry`、RFC 7807 problem response | `ops_audit_events` append-only；時間軸 append-only | bounded-route 單元案例；CO-VRF-API-001／SEC-001／TELEMETRY-001／BROWSER-001 | 指定本機 API 與受控 Log 已驗證；815/815 有效，其中 54 筆為受控 4xx、5xx 為 0；安全負向檢查的 request ID 對應為 44/44，API／schema／deployment version 均一致。正式管線未驗證。 |
