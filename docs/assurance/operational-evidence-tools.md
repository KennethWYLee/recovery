...
目前 37/37 單元測試通過；整體 line、branch、function coverage 分別為 91.72%、84%、92.66%。複雜度門檻 15 目前有 5 個診斷項目；這些項目用來安排檢視，不是單獨的功能失敗。現有 coverage 輸出無法可靠對回相同函式的複雜度，因此不計算 CRAP，避免產生不可重現的數字。
