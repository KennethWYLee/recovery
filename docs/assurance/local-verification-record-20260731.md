...
目前狀態：這是本機候選版檢查點。既有 API、資安、瀏覽器、黑箱、短時負載、受控 Log 與資料庫未就緒後恢復結果綁定舊 Worker digest；新的 fresh-D1 runtime bootstrap 與校內角色選擇另綁定新 Worker digest。兩組證據不得混用。其他結果各自只支持明列的來源、migration、Log 或本機環境；文件修訂不會把本機結果提升為 CI、遠端、正式獨立或外部證據。
...
| 校內角色選擇 runtime-bootstrap Worker | SHA-256 `af852b995266c853facb3d198bd8667198b62c6a7ca6431e35d1152508127286` |
...
| `npm run test:quality` | 40/40；coverage 門檻通過；line 92.17%、branch 85.81%、function 93.97%；5 個複雜度診斷項目；CRAP 未計算 | 指定六個單元測試套件與六個 source 的整體 coverage，以及複雜度超過 15 的檢視位置。 | 測試斷言品質、未納入的 Worker／UI／migration、功能正確性、資安或發布準備度。 |
...
| `npm run test:runtime-bootstrap:local` | 通過；3 phases、39／39／33 queries；4 個非管理員選項；完成 `commander → observer` | 隔離本機 Worker／合成 D1 下，並行首次請求只建立 1 user／membership／audit；`admin` 不在選項且提交被拒絕；伺服器權限隨角色切換；唯讀 5/5 讀取、4/4 method 拒絕、成員目錄 403、audit email 省略；既有 admin、其他網域與 suspended 邊界維持。 | Hosted identity edge 防偽、Sites／production、完整角色 × 事件責任矩陣、admin actor email 或完整存取政策。 |
...
重新建置時，vinext 會產生新的 build ID 與 draft-mode secret，因此同一來源的 Worker digest 可能改變。本紀錄不主張 bit-for-bit reproducible build；既有完整檢查與新的校內角色選擇 runtime-bootstrap 分屬不同的明列 SHA-256，不能互相補足。舊 clean-room 與 manifest 也不能自動延伸到修改後來源。要主張可重現建置，仍須在獨立 remote clone 與已記錄的 CI 環境控制建置輸入並比較結果。
...
- `CO-VRF-RUNTIME-BOOTSTRAP-001`：`verified_local_controlled`。除三階段 fresh-D1 bootstrap 外，本次涵蓋四個非管理員角色選項、管理員排除、`commander → observer` 切換及伺服器權限變化；也保留唯讀 5 個模組、4 個 mutation method、並行首次建立、資料最小化、既有 admin、其他網域及 suspended 邊界查核。這不是 hosted identity edge、production 或完整角色 × 事件責任矩陣。
