...
目前狀態：這是未提交工作目錄的本機候選版檢查點。與 Worker artifact 有關的 API、資安、瀏覽器、黑箱、短時負載及資料庫未就緒後恢復結果，均綁定本文所列 Worker digest；其他結果各自只支持明列的來源、migration、Log 或本機環境。clean-room 已完成有界限的本機隔離快照驗證。文件修訂不會把本機結果提升為 CI、遠端、正式獨立或外部證據。
...
| 產品／API | `2.2.0` |
| schema | `0004` |
| 最終受驗 Worker | SHA-256 `1d8b2d0c99000fb1d6b58b23de97adadba06210f370ddcb03e41c95468cf9158` |
...
| `npm run test:unit` | 29/29 通過 | 授權、領域規則、角色相容性、事件篩選、服務生命週期、時間／DST、簽章 cursor 與 API 輸入邊界的現有單元案例通過。 | D1 trigger、完整使用流程或所有邊界情境。 |
...
遙測分析第一次執行時有 7/363 筆被判定無效。原因不是原始 Log 格式錯誤，而是分析器的 `SAFE_ROUTES` 未登錄服務生命週期歷程 route template。補上允許的 template 並增加對應測試後，以同一份 Log 重跑為 399/399 筆通過；本次部署前重新執行 API、黑箱、負載與瀏覽器流程後，最終證據為 879/879 筆有效，API、schema 與 deployment mismatch 均為 0。這只驗證受控本機 Log 與分析器契約，不代表正式收集、保存或告警已完成。
...
- 從乾淨 Git checkout 進行 commit-bound 或 bit-for-bit 可重現建置；本次 clean-room 使用未提交工作目錄的複本，不能支持這項主張。
