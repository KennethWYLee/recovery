...
- Ready 狀態下先查既有會員資格。既有 `admin` 維持系統管理，不列入校內帳號自選角色；既有 `suspended` 會員維持停用，不得因符合校內網域而重新啟用。
- 只有正規化後精確屬於 `@ntub.edu.tw` 的已驗證帳號，才可在首次登入建立啟用中的唯讀會員，並在每次從登入入口進入時選擇 `commander`、`responder`、`observer` 或 `auditor`。相似網域、子網域及其他網域的未受邀帳號均回傳 403。
- 自選角色更新必須使用目前會員版本並留下稽核紀錄；若使用者仍有進行中的事件責任，只能選擇與該責任相容的角色。
...
- 變更後以既有管理員、校內各可選角色、`suspended` 會員及其他網域未受邀帳號執行受控登入；逐一查核角色選項、伺服器權限、不相容責任的切換拒絕、管理員不可自選、成員目錄與 actor email 投影，並查核稽核記錄。
...
- 已驗證且尚無會員資格的身分，只有精確 `@ntub.edu.tw` 網域會建立初始唯讀會員；校內登入只提供四種非管理員角色，`admin` 不可自選。其他網域未受邀者回傳 403，既有 `suspended` 會員不走自動建立流程。認證失敗不建立不可信 actor 稽核，但 request telemetry 仍應記錄有界 route 與結果。
