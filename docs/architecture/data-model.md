...
| 資料群 | 用途 | 重要不變條件 |
|---|---|---|
| 組織、使用者、會員 | 決定登入後的組織與基礎權限；精確 `@ntub.edu.tw` 的無會員帳號可在首次登入建立唯讀會員，之後於登入流程選擇非管理員角色。 | Email 唯一；一位使用者在組織內只有一筆會員；`suspended` 不復原；自動建立角色只能是 `observer`／`auditor`；自選角色不包含 `admin`，且需符合啟用中事件指派；最後一位啟用管理員不能失去資格；會員更新需版本加一。 |
