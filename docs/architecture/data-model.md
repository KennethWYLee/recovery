...
| 資料群 | 用途 | 重要不變條件 |
|---|---|---|
| 組織、使用者、會員 | 決定登入後的組織與基礎權限。 | Email 唯一；一位使用者在組織內只有一筆會員；最後一位啟用管理員不能失去資格；會員更新需版本加一。 |
...
目前資料庫保存 Email、顯示名稱、角色、操作者、自由文字事件紀錄及外部參考 URL。這些可能構成個資、內部營運資訊或敏感事件內容。產品已有輸入長度、稽核 payload 最小化與秘密不進 telemetry 的設計，但目前沒有已核准的保存期間、刪除程序、資料匯出、合法依據或正式隱私查核結果。
