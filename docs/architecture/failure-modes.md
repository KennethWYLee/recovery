...
| FM-14 | Log、稽核或存取政策回應洩漏秘密／個資 | 憑證、actor email 或成員目錄暴露 | 搜尋 token、cookie、Authorization、body 與自由文字；以 `observer`／`auditor` 查稽核及存取政策回應 | route template、欄位白名單、拒絕 audit 不含 payload；唯讀角色的稽核回應省略 actor email，存取政策只回本人；`admin` 保留 actor email | 隔離本機 Worker／D1 已驗證校內唯讀稽核省略 actor email、成員目錄為 403；正式邊緣的實際第二個 NTUB 帳號、admin actor email、Log、Git history／binary／runtime secret 與保存政策仍待查核。 |
...
| FM-17 | UI 深層連結或前端權限顯示被利用繞過授權 | 唯讀使用者執行寫入或讀到成員目錄／actor email | 直接呼叫 API、修改 query，並以 `observer`／`auditor` 逐一送出 `POST`、`PUT`、`PATCH`、`DELETE` | 伺服器每次查身分、會員、組織權限、事件存取、HTTP method 與回應欄位 | 單元、Gherkin 與隔離本機 Worker／D1 的受控驗證已通過；完整瀏覽器與正式 edge 的實際第二個 NTUB 帳號仍待查核。 |
