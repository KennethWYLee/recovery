...
export default async function CoursePage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  if (!identity) {
    return <main className="classroom-auth-page"><section className="classroom-auth-card"><h1>請先登入</h1><p>完成校內帳號驗證後，系統會帶您回到這門課程。</p><a className="button primary wide" href={chatGPTSignInPath(returnTo)}>使用校內帳號登入</a></section></main>;
  }
