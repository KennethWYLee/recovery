...
目前正從 Continuity Ops 轉型為大學課堂使用的「小組回答與全班完整排序」系統。主要入口是 `/courses`；舊營運介面暫時保留於 `/operations`，避免轉型期間直接破壞既有資料。

第一階段已完成多課程入口、六門預設課程，以及課程新增、重新命名與軟刪除。當日課堂 QR Code、平衡隨機分組、教師拖曳調整、小組回答與個人排序將依 `PROJECT.md` 中已確認的規則繼續實作。

## 第一階段狀態

- 已實作：課程資料模型、教師權限、課程列表、課程工作區、課程新增、改名、刪除與操作紀錄。
- 已驗證：六門預設課程、課程名稱正規化、學期判定、資料庫唯一性與外鍵、API 新增／改名／刪除流程。
- 尚未實作：學生課堂 QR Code、課堂場次、分組、回答、排序、結果與匯出。

## 舊版 Continuity Ops 說明
...
- 營運頁面提供可重複開啟的情境導覽。第一條五步導覽使用只存在於瀏覽器記憶體的固定模擬資料，帶使用者比較更新時間、錯誤、延遲、API／Log 與恢復條件；導覽不寫入 D1，也不把模擬結果列為正式監控證據。
